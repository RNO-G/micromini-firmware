...
static const char * get_name(int d)
{
  switch(d)
  {
    case UART_DESC: return "UART";
    case USB_DESC: return "USB";
...
static void cb_uart(const struct usart_async_descriptor * dev)
{
  (void) dev;
  cb_called[UART_DESC]++;
}

static void cb_uart_tx(const struct usart_async_descriptor * dev)
{
  (void) dev;
  tx_in_progress[UART_DESC]=0;
  d_write(UART_DESC,0,0);
}


static void cb_uart_err(const struct usart_async_descriptor * dev)
{
  (void) dev;
  err_called[UART_DESC]++;
}
...
void io_init()
{
  ...
  desc[UART_DESC] = &EXT_USART;

  if (!callbacks_set)
  {
    usart_async_register_callback(&EXT_USART, USART_ASYNC_RXC_CB, cb_uart);
    usart_async_register_callback(&EXT_USART, USART_ASYNC_TXC_CB, cb_uart_tx);
    usart_async_register_callback(&EXT_USART, USART_ASYNC_ERROR_CB, cb_uart_err);
    callbacks_set = 1;
  }
  usart_async_enable(&EXT_USART);
  uart_init[UART_DESC] = 1;

}

void sbc_io_deinit()
{
  usart_async_disable(&EXT_USART);
  uart_init[UART_DESC]=0;
  tx_in_progress[UART_DESC]=0;
  usart_async_flush_rx_buffer(&EXT_USART);
