...
//convenience
#define CS_ON  gpio_set_pin_level(SPIFLASH_CS, false)
#define CS_OFF gpio_set_pin_level(SPIFLASH_CS, true)
...
const uint32_t config_block_magic = (0xacf9 << 16 ) | (CONFIG_BLOCK_VERSION& 0xffff) ;
const int n_config_slots = 8; //32 kB
...
static void default_init_cfg(config_block_t * cfg)
{
  cfg->micromini_id = 999;
  cfg->lora_stats_interval = 600;
}


static int verify_cfg(config_block_t * cfg)
{
  (void) cfg; 
  return 0;
}
...
static int spi_flash_read_config_block(config_block_t * config_block)
{
  ...
  if (app_test != config_block_magic)
  {
    default_init_cfg(config_block);
    config_block_sync();
    return -2;
...
static void spi_flash_write_config_block(const config_block_t * block)
{
  ...
  _spi_flash_write(next_config_block * block_size, sizeof(config_block_magic), (uint8_t *) config_block);
...
#define application_size  (256-8)*1024
