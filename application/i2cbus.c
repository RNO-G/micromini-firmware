...
int i2c_enqueue(i2c_task_t * task)
{
  ...
  task->done = 0;
  i2c_m_sync_set_slaveaddr(&I2C_HOST, task->addr, I2C_M_SEVEN);
  ...
  if (task->write)
  {
    ...
    ret = i2c_m_sync_transfer(&I2C_HOST, &msg);
  ...
  else
  {
    ...
    {
      ret = i2c_m_sync_cmd_read(&I2C_HOST, task->reg, &task->data, 1);
    }

  }

  if (ret < 0) i2c_m_sync_send_stop(&I2C_HOST); //this should only be necessary on error, I think?
...
int i2c_detect(uint8_t start_addr, uint8_t end_addr, uint8_t *bitset)
{
  ...
  {
    i2c_m_sync_set_slaveaddr(&I2C_HOST, addr, I2C_M_SEVEN);
    int ret = i2c_m_sync_cmd_read(&I2C_HOST,0,&dummy,1);
    if (!ret)
    {
      if (bitset)
      {
...
int i2c_unstick(int ncycles)
{
  ...
  int nones = 0;
	gpio_set_pin_direction(I2C_HOST_SDA, GPIO_DIRECTION_IN);
	gpio_set_pin_direction(I2C_HOST_SCL, GPIO_DIRECTION_OUT);
	gpio_set_pin_level(I2C_HOST_SCL, 0);

  for (int i = 0; i < ncycles; i++)
  {
    delay_us(10);
    gpio_set_pin_level(I2C_HOST_SCL, 1);
    delay_us(5);
    nones += gpio_get_pin_level(I2C_HOST_SDA);
    delay_us(5);
    gpio_set_pin_level(I2C_HOST_SCL, 0);
  }
